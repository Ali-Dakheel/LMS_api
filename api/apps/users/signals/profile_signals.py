    ...
    try:
        with transaction.atomic():
            ...
            if instance.role == 'teacher':
                profile_model, profile_created = TeacherInfo.objects.get_or_create(
                    user=instance
                )
                if profile_created:
                    logger.info(
                ...
                profile_model, profile_created = StudentInfo.objects.get_or_create(
                    user=instance
                )
                if profile_created:
                    logger.info(
            ...
            if not profile_created and instance.role in ['teacher', 'dean', 'student']:
                logger.warning(
                    f"⚠️ Profile already existed for {instance.role} {instance.id}"
                )
    
    except IntegrityError as e:
        # Profile already exists (race condition)
        logger.warning(
            f"⚠️ Profile already exists for user {instance.id} "
...
def log_teacher_profile_update(sender, instance, created, **kwargs):
    """
...
def log_student_profile_update(sender, instance, created, **kwargs):
    """
...
def log_user_deletion(sender, instance, **kwargs):
    ...
    """
    logger.warning(
        f"🗑️ User deleted: {instance.email} "
